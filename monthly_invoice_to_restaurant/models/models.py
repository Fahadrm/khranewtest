    ...
    def _generate_monthly_restaurant_invoice(self):
        ...
        for restaurant in restaurants:
            ...
            if monthly_payments:
                ...
                invoice.action_post()
                if self.env['ir.config_parameter'].sudo().get_param('monthly_invoice_to_restaurant.send_invoice_to_restaurant'):
                    template = self.env.ref('account.email_template_edi_invoice', raise_if_not_found=False)
                    if template:
                        template.send_mail(invoice.id, force_send=True, raise_exception=False, notif_layout=False)
            for payment in monthly_payments:
